...
void update(float t, double temp[500][500], double rchange[500][500] ) {
    int i;
    int j;
...
#pragma omp for
    ...
    for (i = 0; i < 500; i++) {
        ...
        for (j = 0; j < 500; j++) {
            ...
            temp[i][j] = temp[i][j] + t * rchange[i][j];
...
#pragma omp parallel shared(temperatures,rate_of_change)
    {
